...
// Shared repo display. Every entry path (owner/repo, a raw commit-table PDA,
// or a .sol domain) resolves to a commit list + tree, and renders here. Write
// actions (edit / new file / deploy tab) only show when `canEdit` is set and a
// GitClient + repoName are provided — a .sol/PDA visitor gets read-only.
...
export interface RepoViewProps {
  /** Display name — repo.name for owner/repo entry, the domain for .sol. */
  repoLabel: string;
  /** Owner wallet, for the header. Falls back to the latest commit's author. */
  owner?: string;
  commits: Commit[];
  commitsLoading: boolean;
  /** iqpages deployment marker, when known. */
  deployed?: boolean;
  isPublic?: boolean;
  /** Write actions (edit / new file / deploy tab) render only when true and a
   *  client + repoName are present. */
  canEdit?: boolean;
...
export function RepoView({
  ...
  useEffect(() => {
    ...
    const readme = Object.entries(treeQuery.data).find(([p]) => p.toLowerCase() === "readme.md");
    if (readme) {
  ...
  const tabs = [
    ...
    ...(editable ? [{ id: "deployments", label: "DEPLOY", icon: <Globe size={16} /> }] : []),
  ];
  ...
  return (
    <div className="min-h-screen bg-cyber-bg text-foreground font-sans relative overflow-x-hidden">
          ...
          <div className="flex items-center gap-4 flex-1">
            <div className="w-10 h-10 border border-neon-pink bg-neon-pink/10 flex items-center justify-center shadow-[0_0_10px_rgba(255,0,255,0.3)]">
              <span className="font-bold text-lg text-neon-pink">
                {repoLabel.substring(0, 2).toUpperCase()}
              </span>
            </div>
            <div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 relative z-10">
        ...
        {activeTab === "deployments" && editable && (
            ...
            <div className="cyber-card p-8 border-neon-pink/50 bg-black/40">
              ...
              {isPublic ? (
                <>
                  ...
                  >
                    <Globe size={16} />
                    {deployed ? "VIEW_ON_PAGES" : "CONFIGURE_&_DEPLOY"}
                  </Link>
                </>
              ) : (
